...
const Layout: React.FC<LayoutProps> = React.memo(({ children, onLogout, language, onLanguageChange, currentUser, onUserSwitch, onManageAuthorization, theme, onThemeChange }) => {
  ...
  const menu = useMemo(() => (
    <Menu
      ...
      theme={theme}
    />
  ), [isMobile, location.pathname, handleMenuClick, menuItems, theme]);
  ...
  return (
    <ConfigProvider theme={themeConfig}>
      <AntLayout style={{ minHeight: '100vh', background: theme === 'light' ? '#f5f5f5' : '#141414' }}>
      {!isMobile && (
        <Sider
