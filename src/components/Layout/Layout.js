...
import { Layout as AntLayout, Menu, Button, Drawer, Select, Avatar, Switch, ConfigProvider } from 'antd';
import { DashboardOutlined, StockOutlined, TransactionOutlined, NotificationOutlined, ThunderboltOutlined, LineChartOutlined, MessageOutlined, RiseOutlined, PieChartOutlined, DatabaseOutlined, SettingOutlined, LogoutOutlined, MenuOutlined, BellOutlined, LeftOutlined, RightOutlined, MoonOutlined, SunOutlined } from '@ant-design/icons';
import { useNavigate, useLocation } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
const { Header, Sider, Content } = AntLayout;
const Layout = React.memo(({ children, onLogout, language, onLanguageChange, currentUser, onUserSwitch, onManageAuthorization, theme, onThemeChange }) => {
    ...
    const menu = useMemo(() => (_jsx(Menu, { mode: isMobile ? 'vertical' : 'inline', selectedKeys: [location.pathname], onClick: handleMenuClick, items: menuItems, theme: theme })), [isMobile, location.pathname, handleMenuClick, menuItems, theme]);
    ...
    return (_jsx(ConfigProvider, { theme: themeConfig, children: _jsxs(AntLayout, { style: { minHeight: '100vh', background: theme === 'light' ? '#f5f5f5' : '#141414' }, children: [!isMobile && (_jsxs(Sider, { width: isTablet ? 120 : 150, collapsed: collapsed, style: {
                        background: theme === 'light' ? '#fff' : '#1f1f1f',
                        boxShadow: theme === 'light' ? '2px 0 8px rgba(0,0,0,0.1)' : '2px 0 8px rgba(0,0,0,0.3)',
                        overflow: 'auto',
                        height: '100vh',
                        position: 'fixed',
                        left: 0,
                        ...
                        WebkitOverflowScrolling: 'touch'
                                ...
                                bottom: 0,
                                background: theme === 'light' ? '#fff' : '#1f1f1f',
                                borderTop: theme === 'light' ? '1px solid #f0f0f0' : '1px solid #333',
                                width: '100%'
