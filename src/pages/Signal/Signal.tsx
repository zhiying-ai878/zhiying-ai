...
const Signal = () => {
  ...
  const technicalChartInstance = useRef<echarts.ECharts | null>(null);
  ...
  useEffect(() => {
    ...
    const signalListener = () => {
      console.log('信号变化监听器被触发，重新加载信号');
      loadSignals();
    };
    
    signalManager.addListener(signalListener);
    console.log('信号监听器已添加');
    ...
    return () => {
      ...
      console.log('信号监听器已移除');
      // 注意：数据源连接由全局管理，这里不需要停止数据源
      console.log('Signal页面卸载，数据源继续保持连接（全局管理）');
    };
  }, []);
  ...
  const loadSignals = () => {
    ...
    let filteredHistory = history.filter(signal => {
      ...
      if (filterType === 'buy') {
        // 普通买入信号：35/61，置信度100分
        return isBuySignal && signal.confidence >= 100 && hasEnoughConditions && !isSpecialSignal;
      } else if (filterType === 'sell') {
        // 卖出信号：不限制条件
        return signalType === 'sell';
      } else {
        // all - 只显示普通买入和卖出，不包含特殊信号
        return ((isBuySignal && signal.confidence >= 100 && hasEnoughConditions && !isSpecialSignal) || signalType === 'sell');
      }
    });
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const responseTime = Date.now() - startTime; // 在成功后计算响应时间
          setDataSourceStatus({
            isConnected: true,
            lastCheckTime: Date.now(),
            responseTime: responseTime,
            ...
            <Col xs={12} sm={8}>
              <Space size="small">
                ...
                >
                  ...
                  <Select.Option value="buy">买入信号</Select.Option>
