...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleLoginFinish = async (values: any) => {
    ...
    try {
      ...
      console.log('localStorage中的用户列表:', savedUsers);
      
      const user = savedUsers.find((u: any) => u.username === values.username && u.password === values.password);
      console.log('在localStorage中找到的用户:', user);
      
      // 优先使用localStorage中的用户数据
      if (user) {
        console.log('用户存在于localStorage，检查授权状态');
        // 检查用户是否已授权
        if (user.isAuthorized || user.username === '15983768460') {
          console.log('用户已授权，执行登录');
          message.success('登录成功！');
          console.log('调用onLogin:', values.username);
          onLogin(values.username, values.password);
        } else {
          console.log('用户未授权');
          message.error('该账号未授权，请联系管理员授权');
        }
      } 
      // 其次检查硬编码的特殊账号（管理员账号）
      else if (values.username === '15983768460' && values.password === 'admin123') {
        console.log('使用硬编码管理员账号登录');
        message.success('登录成功！');
        console.log('调用onLogin:', values.username);
        onLogin(values.username, values.password);
      } 
