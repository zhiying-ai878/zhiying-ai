...
const Dashboard = React.memo(() => {
        ...
        try {
            ...
            try {
                ...
                if (indexResults && indexResults.length > 0) {
                    ...
                    // 创建代码到数据的映射（支持带前缀和不带前缀的代码），并过滤异常数据
                    const indexMap = new Map();
                    indexResults.forEach(r => {
                        // 过滤异常的指数数据
                        const isIndex = r.code.startsWith('sh000') || r.code.startsWith('000') || 
                                       r.code.startsWith('sz399') || r.code.startsWith('399');
                        const isPriceAbnormal = isIndex && r.price < 100;
                        ...
                        if (!isPriceAbnormal) {
                            ...
                            console.log(`跳过异常指数数据: ${r.name}(${r.code}) - 价格 ${r.price} 异常`);
                        }
                    });
                    console.log('指数映射:', Array.from(indexMap.keys()));
                    
                    // 获取指数数据，异常数据时使用当前值保持稳定
                    const getIndexValue = (code, field, defaultValue = 0) => {
                        const data = indexMap.get(code) || indexMap.get(code.substring(2));
                        if (data) {
                            // 对于指数，验证数据合理性
                            if (field === 'price') {
                                if (code === 'sh000001' && data.price < 100) return defaultValue;
                                if (code === 'sh000688' && data.price < 100) return defaultValue;
                                if (code === 'sz399001' && data.price < 1000) return defaultValue;
                                if (code === 'sz399006' && data.price < 100) return defaultValue;
                            }
                            return data[field];
                        }
                        return defaultValue;
                    ...
                    const updatedMarketData = [
                        { name: '上证指数', value: getIndexValue('sh000001', 'price', marketData[0]?.value || 3000), change: getIndexValue('sh000001', 'change', marketData[0]?.change || 0), changePercent: getIndexValue('sh000001', 'changePercent', marketData[0]?.changePercent || 0) },
                        { name: '深证成指', value: getIndexValue('sz399001', 'price', marketData[1]?.value || 10000), change: getIndexValue('sz399001', 'change', marketData[1]?.change || 0), changePercent: getIndexValue('sz399001', 'changePercent', marketData[1]?.changePercent || 0) },
                        { name: '创业板指', value: getIndexValue('sz399006', 'price', marketData[2]?.value || 2000), change: getIndexValue('sz399006', 'change', marketData[2]?.change || 0), changePercent: getIndexValue('sz399006', 'changePercent', marketData[2]?.changePercent || 0) },
                        { name: '科创综指', value: getIndexValue('sh000688', 'price', marketData[3]?.value || 1000), change: getIndexValue('sh000688', 'change', marketData[3]?.change || 0), changePercent: getIndexValue('sh000688', 'changePercent', marketData[3]?.changePercent || 0) },
                    ];
    ...
    const loadLatestSignals = useCallback(() => {
        try {
            ...
            // 统一过滤条件：35/61，置信度100，卖出信号不限制
            const filtered = signalHistory.filter(signal => {
                if (signal.type === 'sell') {
                    // 卖出信号不限制条件
                    return true;
                }
                // 买入/特殊信号需要满足35/61，置信度>=100
                return signal.confidence >= 100 && signal.satisfiedConditions >= 35;
            });
