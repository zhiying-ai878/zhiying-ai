...
export interface FeatureConfig {
  ...
  includeVolatilityFeatures: boolean;
  includeTrendFeatures: boolean;
...
export class FeatureEngineer {
  ...
  // 提取股票特征
  extractFeatures(historicalData: HistoricalData[]): StockFeatures[] {
  ...
  // 计算移动平均线
  private calculateMovingAverages(data: HistoricalData[], index: number, feature: StockFeatures) {
    ...
    this.config.lookBackPeriods.ma.forEach(period => {
      if (index >= period - 1) {
        ...
        switch (period) {
          case 5: feature.ma5 = parseFloat(ma.toFixed(2)); break;
          case 10: feature.ma10 = parseFloat(ma.toFixed(2)); break;
          case 20: feature.ma20 = parseFloat(ma.toFixed(2)); break;
          case 30: feature.ma30 = parseFloat(ma.toFixed(2)); break;
          case 60: feature.ma60 = parseFloat(ma.toFixed(2)); break;
        }
      }
    });
  }
  ...
  // 计算MACD
  private calculateMACD(data: HistoricalData[]): Array<{ macd: number; signal: number; histogram: number }>{
    const prices = data.map(d => d.close);
    const results: Array<{ macd: number; signal: number; histogram: number }>= [];
  ...
  // 计算KDJ
  private calculateKDJ(data: HistoricalData[]): Array<{ k: number; d: number; j: number }>{
    const results: Array<{ k: number; d: number; j: number }>= [];
  ...
  // 计算布林带
  private calculateBollingerBands(data: HistoricalData[]): Array<{ upper: number; middle: number; lower: number; bandwidth: number }>{
    const results: Array<{ upper: number; middle: number; lower: number; bandwidth: number }>= [];
    const prices = data.map(d => d.close);
  ...
  // 计算ATR
  private calculateATR(data: HistoricalData[]): number {
    const ranges: number[] = [];
  ...
  // 计算趋势
  private calculateTrend(data: HistoricalData[]): { strength: number; direction: 'up' | 'down' | 'sideways' } {
    const prices = data.map(d => d.close);
  ...
  // 计算市场情绪
  private calculateSentiment(feature: StockFeatures): number {
    let score = 50; // 中性情绪
    
    // RSI影响
    if (feature.rsi > 70) score += (feature.rsi - 70) * 0.5;
    else if (feature.rsi< 30) score -= (30 - feature.rsi) * 0.5;
    
    // MACD影响
    if (feature.macd.macd >feature.macd.signal) score += 5;
    else score -= 5;
    
    // 趋势影响
    if (feature.trendDirection === 'up') score += feature.trendStrength * 2;
    else if (feature.trendDirection === 'down') score -= feature.trendStrength * 2;
    
    // 量价关系影响
    if (feature.priceVolumeCorrelation > 0.5) score += 10;
    else if (feature.priceVolumeCorrelation< -0.5) score -= 10;
  ...
  // 批量提取特征
  extractBatchFeatures(historicalDataMap: Map<string, HistoricalData[]>): Map<string, StockFeatures[]>{
    const results = new Map<string, StockFeatures[]>();
  ...
  // 获取特征统计信息
  getFeatureStatistics(features: StockFeatures[]): any {
...
// 全局实例
let featureEngineerInstance: FeatureEngineer | null = null;

export function getFeatureEngineer(config?: Partial<FeatureConfig>): FeatureEngineer {
