...
export interface PriceVolumeRelation {
  priceVolumeCorrelation: number;
  volumePriceDivergence: 'bullish' | 'bearish' | 'neutral';
...
export interface PredictionResult {
  ...
  technicalIndicators: TechnicalIndicatorSignals;
  priceVolumeAnalysis: PriceVolumeSignals;
...
export interface TechnicalIndicatorSignals {
  macdSignal: 'bullish' | 'bearish' | 'neutral';
  rsiSignal: 'oversold' | 'overbought' | 'neutral';
  kdjSignal: 'goldenCross' | 'deathCross' | 'neutral';
  bollingerSignal: 'breakout' | 'breakdown' | 'neutral';
  maSignal: 'bullish' | 'bearish' | 'neutral';
}

export interface PriceVolumeSignals {
  volumeConfirmation: boolean;
  divergenceSignal: 'bullish' | 'bearish' | 'neutral';
  accumulationSignal: 'accumulating' | 'distributing' | 'neutral';
}

export interface MLPrediction {
  modelType: 'ensemble' | 'lstm' | 'randomForest';
...
export interface PredictionConfig {
  ...
  useHistoricalAnalysis: boolean;
  useTechnicalIndicators: boolean;
  usePriceVolume: boolean;
  useML: boolean;
}
...
class PricePredictionManager {
  private static instance: PricePredictionManager;
  private historicalData: Map<string, HistoricalPriceData[]> = new Map();
  private predictions: Map<string, PredictionResult[]> = new Map();
  private config: PredictionConfig = DEFAULT_CONFIG;
  private listeners: ((predictions: PredictionResult[]) => void)[] = [];

  private constructor() {}

  static getInstance(): PricePredictionManager {
  ...
  addHistoricalData(stockCode: string, data: HistoricalPriceData[]) {
    this.historicalData.set(stockCode, data);
  }

  getHistoricalData(stockCode: string): HistoricalPriceData[] {
    return this.historicalData.get(stockCode) || [];
  }

  generatePrediction(stockCode: string, stockName: string): PredictionResult | null {
    ...
    // 5. 综合分析生成预测
    const { predictionType, confidence, reasons, predictedChange } = this.generateFinalPrediction(
      historicalAnalysis, 
      technicalIndicators, 
      priceVolumeAnalysis, 
      mlPrediction
  ...
  private analyzeHistoricalData(prices: HistoricalPriceData[]): HistoricalAnalysis {
  ...
  private calculateTechnicalIndicators(prices: HistoricalPriceData[]): TechnicalIndicatorSignals {
  ...
  private analyzePriceVolume(prices: HistoricalPriceData[]): PriceVolumeSignals {
    const closePrices = prices.map(p => p.close);
    ...
    // 量价背离
    let volumePriceDivergence: 'bullish' | 'bearish' | 'neutral' = 'neutral';
    ...
    // 累积信号
    let accumulationSignal: 'accumulating' | 'distributing' | 'neutral' = 'neutral';
  ...
  private performMLPrediction(prices: HistoricalPriceData[], historicalAnalysis: HistoricalAnalysis, technicalIndicators: TechnicalIndicatorSignals): MLPrediction {
    // 模拟机器学习预测
  ...
  private generateFinalPrediction(historicalAnalysis: HistoricalAnalysis, technicalIndicators: TechnicalIndicatorSignals, priceVolumeAnalysis: PriceVolumeSignals, mlPrediction: MLPrediction): {
    predictionType: 'buy' | 'sell' | 'hold';
    confidence: number;
    reasons: string[];
    predictedChange: number;
    ...
    // 确定预测类型
    let predictionType: 'buy' | 'sell' | 'hold' = 'hold';
    if (score > 30) {
      predictionType = 'buy';
    } else if (score < -30) {
      predictionType = 'sell';
  ...
  private calculateKDJ(prices: HistoricalPriceData[], period: number): {
    k: number[];
    d: number[];
    j: number[];
  } {
  ...
  getPredictions(stockCode?: string): PredictionResult[] {
    if (stockCode) {
  ...
  subscribe(listener: (predictions: PredictionResult[]) => void) {
  ...
  updateConfig(newConfig: Partial<PredictionConfig>) {
    this.config = { ...this.config, ...newConfig };
  }

  generateMockHistoricalData(stockCode: string, days = 60): HistoricalPriceData[] {
...
export function getPricePredictionManager(): PricePredictionManager {
