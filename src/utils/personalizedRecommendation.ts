...
import { getHistoryManager, HistoryRecord } from './historyManager';
...
export interface StockRecommendation {
  ...
  score: number;
  reasons: string[];
  category: 'trending' | 'similar' | 'historical' | 'technical' | 'industry' | 'risk_based' | 'recent_activity';
  industry?: string;
...
export interface UserPreferences {
  favoriteStocks: Array<{ code: string; name: string; frequency: number; interaction?: { views: number; analyses: number; ignores: number } }>;
  preferredCategories: Array<{ category: string; score: number }>;
  preferredIndustries: Array<{ industry: string; score: number }>;
  recentActivity: number;
  riskTolerance: 'low' | 'medium' | 'high';
  averageHoldTime: number; // 平均持有时间（天）
  interactionPatterns: Record<string, number>; // 互动模式
  averageReturn: number; // 平均收益率
  tradingFrequency: number; // 交易频率
  investmentHorizon: 'short' | 'medium' | 'long'; // 投资期限偏好
  industryConcentration: number; // 行业集中度
  averageInvestment: number; // 平均投资金额
}
...
// 股票风险等级
const stockRiskMap: Record<string, 'low' | 'medium' | 'high'> = {
  '002594': 'high',
  '300750': 'high',
  '601799': 'medium',
  '000887': 'medium',
...
class PersonalizedRecommendation {
  ...
  // 分析用户偏好
  analyzeUserPreferences(): UserPreferences {
    const records = this.historyManager.getRecords();
    const stockFrequency: Record<string, { name: string; count: number }> = {};
    const categoryScores: Record<string, number> = {};
    const industryScores: Record<string, number> = {};
    const interactionPatterns: Record<string, number> = {};
    const stockInteraction: Record<string, { views: number; analyses: number; ignores: number }> = {};
    let totalHoldTime = 0;
    let holdTimeCount = 0;
    let totalReturn = 0;
    let returnCount = 0;
  ...
  // 生成个性化推荐
  generateRecommendations(): StockRecommendation[] {
  ...
  // 计算推荐评分
  private calculateScore(stockCode: string, preferences: UserPreferences, category: string): number {
    let baseScore = 0.5;
  ...
  // 查找相似股票
  private findSimilarStocks(stockCode: string): Array<{ code: string; name: string }> {
    // 模拟相似股票数据
    const similarStocksMap: Record<string, Array<{ code: string; name: string }>> = {
      '002594': [
  ...
  // 获取行业推荐
  private getIndustryRecommendations(industry: string): StockRecommendation[] {
    const industryStocks: Array<{ code: string; name: string }> = [];
  ...
  // 获取基于风险的推荐
  private getRiskBasedRecommendations(riskTolerance: 'low' | 'medium' | 'high'): StockRecommendation[] {
    const riskStocks: Array<{ code: string; name: string }> = [];
  ...
  // 获取技术分析推荐
  private getTechnicalRecommendations(): StockRecommendation[] {
    // 模拟技术分析推荐
    return [
      {
        stockCode: '600036',
  ...
  // 获取趋势推荐
  private getTrendingStocks(): StockRecommendation[] {
    // 模拟趋势推荐
    return [
      {
        stockCode: '000001',
  ...
  // 获取基于近期活动的推荐
  private getRecentActivityRecommendations(preferences: UserPreferences): StockRecommendation[] {
    // 基于用户近期活动生成推荐
  ...
  // 去重推荐
  private removeDuplicates(recommendations: StockRecommendation[]): StockRecommendation[] {
  ...
  // 记录用户与推荐的互动
  recordRecommendationInteraction(stockCode: string, stockName: string, action: 'view' | 'ignore' | 'analyze'): void {
  ...
  // 获取推荐解释
  getRecommendationExplanation(stockCode: string, preferences: UserPreferences): string {
...
let recommendationInstance: PersonalizedRecommendation | null = null;

export const getPersonalizedRecommendation = (): PersonalizedRecommendation => {
