...
// 支持的模型类型
export type ModelType = 'lstm' | 'prophet' | 'linear' | 'ensemble' | 'xgboost' | 'arima' | 'cnn';
...
export class EnsembleModel extends MachineLearningModel {
  ...
  private modelPerformances: number[] = [];
...
export class XGBoostModel extends MachineLearningModel {
  ...
  private prepareData(data: number[]): { xTrain: number[][]; yTrain: number[] } {
    const x: number[][] = [];
    const y: number[] = [];
  ...
  private trainTree(xTrain: number[][], yTrain: number[]): any {
    // 简化的决策树训练
    // 实际项目中可以使用更复杂的实现
    return {
  ...
  private predictTree(tree: any, input: number[]): number {
...
export class ARIMAModel extends MachineLearningModel {
  private p: number = 2; // 自回归阶数
  private d: number = 1; // 差分阶数
  private q: number = 2; // 移动平均阶数
  private coefficients: any = {};
  ...
  private inverseDifference(originalData: number[], differencedData: number[], d: number): number[] {
...
export const generatePrediction = async (
  ...
  // 尝试从缓存获取
  const cachedResult = await cache.get<PredictionResult>(cacheKey);
  ...
  if (latestRSI > 0) {
    if (latestRSI < 30) technicalScore += 20; // 超卖
    else if (latestRSI > 70) technicalScore -= 20; // 超买
  }
  ...
  if (latestUpper > 0 && latestLower > 0) {
    if (currentPrice < latestLower) technicalScore += 15; // 接近下轨
    else if (currentPrice > latestUpper) technicalScore -= 15; // 接近上轨
  }
  ...
  if (latestMA5 > 0 && latestMA20 > 0) {
    if (latestMA5 > latestMA20) technicalScore += 10; // 短期均线上穿长期均线
    else if (latestMA5 < latestMA20) technicalScore -= 10; // 短期均线下穿长期均线
  }
