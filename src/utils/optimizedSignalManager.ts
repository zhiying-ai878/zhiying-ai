...
class OptimizedSignalManager {
  ...
  private assessMainForceRisk(data: ComprehensiveData): number {
    ...
    // 成交量异常风险
    if (mainForceData.volumeAmplification > 5) riskScore += 15;
    else if (mainForceData.volumeAmplification > 3) riskScore += 10;
    
    // 换手率异常风险
    if (mainForceData.turnoverRate > 20) riskScore += 20;
    else if (mainForceData.turnoverRate > 15) riskScore += 15;
    else if (mainForceData.turnoverRate > 10) riskScore += 10;
...
let signalManager: OptimizedSignalManager | null = null;
