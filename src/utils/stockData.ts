...
export type DataSourceType = 'sina' | 'tencent' | 'eastmoney' | 'xueqiu' | 'ths' | 'mock' | 'huatai' | 'gtja' | 'haitong' | 'wind' | 'choice' | 'tdx' | 'dzh' | 'jrj' | 'p5w';
...
class StockDataSource {
  ...
  private initializeHealthStatus() {
    const sources: DataSourceType[] = ['sina', 'tencent', 'eastmoney', 'xueqiu', 'ths', 'mock', 'huatai', 'gtja', 'haitong', 'wind', 'choice', 'tdx', 'dzh', 'jrj', 'p5w'];
  ...
  private async batchRequest<T>(source: DataSourceType, codes: string[], requestFn: (codes: string[]) => Promise<T[]>): Promise<T[]> {
    ...
    try {
      ...
      const batchResults = await Promise.all(
        batches.map(async (batchCodes) => {
          try {
            ...
            // 为该批次返回模拟数据
            return this.getMockDataForCodes<T>(batchCodes);
          }
        })
      );
      ...
      const mockResults = this.getMockDataForCodes<T>(codes);
  ...
  // 预加载数据
  async preloadData(codes: string[], priority: 'high' | 'medium' | 'low' = 'medium') {
    try {
  ...
  async intelligentBatchRequest<T>(source: DataSourceType, codes: string[], requestFn: (codes: string[]) => Promise<T[]>): Promise<T[]> {
    ...
    try {
      ...
      return [...cacheResults, ...this.getMockDataForCodes<T>(uncachedCodes)];
    }
  }
  ...
  async getFinancialData(codes: string[]): Promise<FinancialData[]> {
    ...
    const cached = await this.getCache<FinancialData[]>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  async getNewsData(keyword?: string, stockCode?: string, count: number = 20): Promise<NewsData[]> {
    ...
    const cached = await this.getCache<NewsData[]>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  async setSourceType(type: DataSourceType) {
  ...
  private async setCache<T>(key: string, data: T, customTTL?: number) {
  ...
  // 批量获取缓存
  private async getBatchCache<T>(keys: string[]): Promise<Map<string, T>> {
  ...
  // 批量设置缓存
  private async setBatchCache<T>(items: Map<string, T>) {
          ...
          if (values.length >= 10) {
            ...
            if (!isNaN(price) && price > 0) {
              ...
              const originalCode = codes.find(c => {
                ...
                if (c.length === 6 && code.endsWith(c)) return true;
  ...
  private async getTDXRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    const results: StockQuote[] = [];
    const startTime = Date.now();
  ...
  private async getDZHRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    const results: StockQuote[] = [];
    const startTime = Date.now();
  ...
  private async getJRJRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    const results: StockQuote[] = [];
    const startTime = Date.now();
  ...
  private async getP5WRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    const results: StockQuote[] = [];
    const startTime = Date.now();
  ...
  // 数据质量监控
  private monitorDataQuality(data: StockQuote[], source: DataSourceType) {
  ...
  async getRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    ...
    for (const code of codes) {
      // 检查各个数据源的缓存
      const dataSourcePriority: DataSourceType[] = ['sina', 'eastmoney', 'tencent', 'xueqiu', 'ths', 'mock'];
      let foundInCache = false;
      
      for (const source of dataSourcePriority) {
        const cacheKey = this.cache.generateKey(CacheKeys.STOCK_DATA, source, code);
        const cachedData = await this.getCache<StockQuote>(cacheKey);
    ...
    // 强制使用新浪数据源，确保获取真实数据
    const dataSourcePriority: DataSourceType[] = ['sina', 'eastmoney', 'tencent', 'xueqiu', 'ths', 'tdx', 'dzh', 'wind', 'choice', 'jrj', 'p5w'];
    let freshResults: StockQuote[] = [];
    ...
    // 确保返回的数据顺序与输入代码顺序一致
    const orderedResults: StockQuote[] = [];
    for (const code of codes) {
  ...
  // 获取市场状态描述
  getMarketStatus(): 'open' | 'closed' | 'pre_open' | 'post_close' {
    const now = new Date();
    const dayOfWeek = now.getDay();
    const hours = now.getHours();
    const minutes = now.getMinutes();
  ...
  async getStockList(): Promise<StockQuote[]> {
    ...
    const cached = await this.getCache<StockQuote[]>(cacheKey);
  ...
  async getKLineData(code: string, period: 'day' | 'week' | 'month' = 'day', count: number = 60): Promise<KLineData[]> {
    ...
    const cached = await this.getCache<KLineData[]>(cacheKey);
  ...
  async getMainForceData(codes: string[]): Promise<MainForceData[]> {
    ...
    const cached = await this.getCache<MainForceData[]>(cacheKey);
