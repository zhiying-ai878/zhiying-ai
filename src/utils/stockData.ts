...
class StockDataSource {
  ...
  async autoFailover(): Promise<DataSourceType> {
    ...
    // 快速测试所有可用数据源
    const availableSources: { source: DataSourceType; responseTime: number }[] = [];
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          if (values.length >= 30) {
            ...
            // 腾讯API返回的指数数据已经是正确的价格格式（带有小数点），不需要除以100
            const price = priceValue;
            const close = closeValue;
            const open = openValue;
            const high = highValue;
  ...
  private getEnhancedDataSourceList(): Array<{ name: string; method: Function }> {
    ...
    const rankedSources = allSources.map(source => {
      ...
      if (health) {
        switch (health.status) {
          case 'healthy': score += 250; break;
          case 'degraded': score += 80; break;
  ...
  async getStockList(): Promise<any[]> {
    ...
    try {
      ...
      try {
        ...
        const stockListPath = path.join(__dirname, 'stockList.json');
        ...
        console.warn('从本地文件读取股票列表失败:', fileError);
      }
  ...
  private async fetchTencentKLineData(code: string, period: 'day' | 'week' | 'month', count: number): Promise<KLineData[]> {
    try {
      ...
      if (response.data && (response.data as any).data && (response.data as any).data[tencentCode]) {
        const klineData = (response.data as any).data[tencentCode][periodMap[period]];
      ...
      this.logger.error(`腾讯K线数据获取失败:`, err);
      throw err;
    }
  }
  ...
  // 生成模拟K线数据作为降级策略
  private generateMockKLineData(code: string, period: 'day' | 'week' | 'month', count: number): KLineData[] {
    const data: KLineData[] = [];
    ...
    if (code === '600519') { // 贵州茅台
      ...
      basePrice = 100 + Math.random() * 20; // 100-120元
    } else if (code === '301179') { // 泽宇智能
      basePrice = 20 + Math.random() * 10; // 20-30元
  ...
  async getMainForceData(codes: string[]): Promise<MainForceData[]> {
    ...
    try {
      ...
      this.logger.error('所有主力资金数据源都无法获取数据，使用行情数据估算主力资金');
      
      // 如果所有数据源都失败，基于行情数据估算主力资金
      try {
        const stockQuotes = await this.getRealtimeQuote(codes);
        if (stockQuotes.length > 0) {
          const localResults: MainForceData[] = [];
  ...
  async scanAllStocks(batchSize: number = 50): Promise<StockQuote[]> {
    try {
      ...
      const allQuotes = await this.processBatch<StockQuote>(
        stockCodes,
        batchSize,
        (batch) => this.getRealtimeQuote(batch)
      );
...
export const getStockDataSource = (sourceType?: DataSourceType, options?: Partial<DataSourceOptions>): StockDataSource => {
  if (!stockDataSourceInstance) {
    console.log('创建新的数据源实例，类型:', sourceType || 'sina_cors');
    stockDataSourceInstance = new StockDataSource(sourceType || 'sina_cors', options);
  } else if (sourceType) {
