...
class StockDataSource {
  ...
  private async batchRequest<T>(source: DataSourceType, codes: string[], requestFn: (codes: string[]) => Promise<T[]>): Promise<T[]> {
    ...
    try {
        ...
        if (backupSource !== source) {
          try {
            ...
            if (backupResult && backupResult.length > 0) {
              ...
              this.setCache(backupCacheKey, backupResult);
      ...
      this.setCache(mockCacheKey, mockResults, 30000);
  ...
  private setCache<T>(key: string, data: T, customTTL?: number) {
  ...
  async getRealtimeQuote(codes: string[]): Promise<StockQuote[]> {
    ...
    for (const code of codes) {
      ...
      for (const source of dataSourcePriority) {
        const cacheKey = this.cache.generateKey(CacheKeys.STOCK_DATA, source, code);
        const cachedData = this.getCache<StockQuote>(cacheKey);
    ...
    // 强制使用新浪数据源，确保获取真实数据
    const dataSourcePriority: DataSourceType[] = ['sina', 'eastmoney', 'tencent', 'xueqiu', 'ths'];
      ...
      try {
        ...
        if (hasRealData && qualityResult.qualityScore > 70) {
          ...
          if (validResults.length > 0) {
            ...
            // 缓存有效结果
            timestampedResults.forEach(result => {
    ...
    if (freshResults.length === 0) {
      ...
      freshResults.forEach(result => {
        ...
        this.setCache(cacheKey, result, 30000); // 模拟数据缓存30秒
