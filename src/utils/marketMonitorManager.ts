...
class MarketMonitorManager {
      ...
      if (marketStatus === 'closed') {
        ...
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('行情数据获取超时')), 30000)
          );
  ...
  private generateBuySignal(data: any, limitUpPotentialScore: number = 0): any | null {
    ...
    // 优化的买入条件，特别关注底部放量涨停板股票、龙头股票和新股
    const buyConditions = [
      // 主力资金条件（确保能捕捉到资金流入）
      mainForceNetFlow >= -50000, // 允许小幅资金流出
      mainForceRatio >= 0, // 零门槛主力资金占比
      ...
      mainForceData.continuousFlowPeriods >= 1, // 连续流入1期
      mainForceData.continuousFlowPeriods >= 2, // 连续流入2期
      mainForceData.continuousFlowPeriods >= 3, // 连续流入3期
      
      // 价格和技术指标条件（确保所有上涨股票都能生成信号）
      data.changePercent !== undefined && data.changePercent > 0, // 只要上涨就满足
      data.changePercent !== undefined && data.changePercent > 0.01, // 极小涨幅也能触发信号
      data.changePercent !== undefined && data.changePercent > 0.1, // 微小涨幅也能触发信号
      data.changePercent !== undefined && data.changePercent > 0.5, // 小幅上涨也可能是大涨前兆
      data.changePercent !== undefined && data.changePercent > 1, // 小幅上涨
      data.changePercent !== undefined && data.changePercent > 2, // 涨幅超过2%
      data.changePercent !== undefined && data.changePercent > 3, // 涨幅超过3%
      data.changePercent !== undefined && data.changePercent > 5, // 涨幅超过5%
      data.changePercent !== undefined && data.changePercent > 7, // 涨幅超过7%
      data.changePercent !== undefined && data.changePercent > 9, // 涨幅超过9%（接近涨停）
      
      // 成交量和活跃度条件
      mainForceData.volumeAmplification > 1.2, // 成交量放大1.2倍
      mainForceData.volumeAmplification > 1.5, // 成交量放大1.5倍
      mainForceData.volumeAmplification > 2, // 成交量放大2倍
      mainForceData.volumeAmplification > 3, // 成交量放大3倍
      mainForceData.turnoverRate > 3, // 换手率超过3%
      mainForceData.turnoverRate > 5, // 换手率超过5%
      mainForceData.turnoverRate > 10, // 换手率超过10%
      ...
      // 行业和概念条件
      mainForceData.industryRank< 50, // 行业排名前50
      mainForceData.industryRank <30, // 行业排名前30
      mainForceData.industryRank< 20, // 行业排名前20
      mainForceData.conceptRank <50, // 概念排名前50
      mainForceData.conceptRank <30, // 概念排名前30
      
      // 市场类型条件 - 特别关注新股
      data.stockCode && data.stockCode.startsWith('001'), // 001开头新股
      ...
      data.stockName && (data.stockName.startsWith('N') || data.stockName.startsWith('S')), // N/S开头新股
      ...
      mainForceData.volumeAmplification > 3 && mainForceNetFlow > 100000, // 大幅放量+大量资金流入
      
      // 龙头股票特殊条件
      data.changePercent && data.changePercent > 3 && mainForceNetFlow > 100000, // 中等涨幅+大量资金
    ...
    if (satisfiedConditions >= minConditions) {
      ...
      if (data.changePercent) {
        if (data.changePercent > 15) confidence += 40;
        else if (data.changePercent > 10) confidence += 30;
        else if (data.changePercent > 5) confidence += 20;
        else if (data.changePercent > 2) confidence += 10;
      }
      
      // 根据主力资金强度调整置信度
      if (mainForceNetFlow > 500000) confidence += 30;
      else if (mainForceNetFlow > 200000) confidence += 25;
      else if (mainForceNetFlow > 100000) confidence += 20;
      else if (mainForceNetFlow > 50000) confidence += 15;
      else if (mainForceNetFlow > 10000) confidence += 10;
      
      // 根据资金类型调整置信度
      if (mainForceData.mainForceType === 'institution') confidence += 25;
      else if (mainForceData.mainForceType === 'privateFund') confidence += 20;
      
      // 根据连续流入周期调整置信度
      if (mainForceData.continuousFlowPeriods >= 3) confidence += 30;
      else if (mainForceData.continuousFlowPeriods >= 2) confidence += 20;
      else if (mainForceData.continuousFlowPeriods >= 1) confidence += 15;
      
      // 根据技术形态调整置信度
      if (currentPrice > ma.ma5 * 1.02 && currentPrice > ma.ma10 * 1.01) confidence += 15;
      if (macd && macd.diff > macd.dea * 1.1) confidence += 15;
      if (kdj && kdj.k > kdj.d * 1.1) confidence += 15;
