...
const {
  asyncSeqsToArray
} = require('../../internal/async-any-seq.js');
...
const {
  __asyncWindowAhead
} = require('../$window-ahead/async-window-ahead.js');
...
async function* asyncAnySeqspliterator(split, {
  separatorSeqs,
  same
}, source) {
  ...
  for await (const buffer of __asyncWindowAhead(source, maxMatchLength, {
    useFiller: false
  })) {
    if (skip > 0) {
      skip--;
...
async function* __asyncSplitOnAnySeq(source, separatorSeqs, same = Object.is) {
...
const asyncSplitOnAnySeq = /*#__PURE__*/asyncIterableCurry(__asyncSplitOnAnySeq, {
  minArgs: 1,
  maxArgs: 2
});
